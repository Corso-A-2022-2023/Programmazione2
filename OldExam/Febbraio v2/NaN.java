...
Leggete attentamente la definizione del metodo Node g(Node q). Completate il corpo del metodo boolean ok(Node q) in modo che restituisca vero se e solo se eseguire il metodo Node g(Node q) completerebbe senza sollevare eccezioni, nè restituendo valori inf, NaN. Non usate 'catch', ma fornite un algoritmo che decida quando non ci sono eccezioni/inf/NaN. Non modificate altro che il corpo di ok.
...
//METHOD TO FILL IN  
  ...
  { if ( q==null) return false;
    while( q.getNext()!=null &&
           ((q.getElem() <= q.getNext().getElem()) 
            == false)
          ) 
    { q=q.getNext(); }
   return q.getNext()!=null; }
    
 public static Node g(Node q)
  { while( (q.getElem() <= q.getNext().getElem()) 
           == false ) 
     { q=q.getNext(); } 
    return q; } 
